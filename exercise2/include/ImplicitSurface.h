...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
        ...
        return (v.dot(v) - (m_radius*m_radius));
	}
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
        ...
        Eigen::Vector3d v = _x-m_center;
        double func_value = v.dot(v) + (m_radius*m_radius) - (m_a * m_a);
        func_value = func_value * func_value;
        func_value = func_value - (4 * m_radius * m_radius * ((v[0] * v[0]) + (v[1] * v[1])));
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		Eigen::Vector3d b = Eigen::Vector3d(m_coefficents[m_numCenters],m_coefficents[m_numCenters+1],m_coefficents[m_numCenters+2]);

		for(size_t i = 0; i < m_numCenters; i++){
		    result = result + (m_coefficents[i] * EvalBasis((m_funcSamp.m_pos[i]-_x).norm()));
		}

		result = result + (b.dot(_x)) + d;
	...
	void BuildSystem()
	{
		...
		for(size_t i=0; i < 2*m_numCenters; i++){
		    ...
		    for(size_t j=0; j<3; j++){
                A(i, m_numCenters+j) = m_funcSamp.m_pos[i][j];
		    }
